...
export default function PromptAgent() {
  ...
  function removeFile() { setFile(null); if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(null); }
  ...
  function onRun() {
    ...
    push("agent", ["Plan:", ...d.plan.map((s, i) => `${i + 1}. ${s}`)].join("\n"));
  }
  ...
  async function onConfirm() {
    ...
    if (intent.kind === "swap") {
      try {
        ...
        const amountRaw = BigInt(Math.floor(intent.amount * 10 ** dec));
  ...
  slippagePct: intent.slippagePct,
        ...
        setToast("Swap success ✅"); clearPlan();
      } catch (e: any) { push("agent", `Swap error: ${e?.message || e}`); setToast("Swap error ❌"); }
      return;
    }

    if (intent.kind === "register") {
      try {
        if (!file) { setStatus("Butuh gambar. Klik ikon attach."); setToast("Attach image dulu 📎"); return; }
        setStatus("Upload image..."); const fd = new FormData(); fd.append("file", file);
        const upFile = await fetch("/api/ipfs/file", { method: "POST", body: fd }).then(r=>r.json());
        const fileHash = await sha256Hex(file);

        const ipMeta = {
          title: intent.title || file.name, description: intent.prompt || "",
          image: upFile.url, imageHash: fileHash, mediaUrl: upFile.url, mediaHash: fileHash, mediaType: file.type,
        ...
        const res = await client.ipAsset.mintAndRegisterIp({
          ...
          ipMetadata: { ipMetadataURI: upMeta.url, ipMetadataHash: upMeta.hash, nftMetadataURI: upNft.url, nftMetadataHash: upNft.hash },
        });

        setStatus("Done");
        push("agent", `Register success ✅\nipId: ${res.ipId}\nTx: ${res.txHash}\nImage: ${upFile.url}\nMetadata: ${upMeta.url}\n↗ View: ${explorerBase}/tx/${res.txHash}`);
        setToast("IP registered ✅"); clearPlan();
      } catch (e: any) { push("agent", `Register error: ${e?.message || e}`); setToast("Register error ❌"); }
    }
  }
  ...
  {messages
    ...
    .map((m: Msg, i: number) => (
      <li key={i} className="text-sm text-white/90">{m.text}</li>
    ))}
</ul>
      </aside>
      ...
      <section className="space-y-4">
        <div ref={chatRef} className="card h-[360px] overflow-auto scrollbar-invisible">
          {messages.length===0
            ? <div className="text-xs text-white/50">AI replies will appear here. Try: <span className="badge">Swap 1 WIP &gt; USDC slippage 0.5%</span> or <span className="badge">Register this image IP, title "Sunset" by-nc</span>.</div>
            : <div className="space-y-3">
                {messages.map((m: Msg, i: number) => (
  <div key={i} className={`flex ${m.role === "you" ? "justify-end" : "justify-start"}`}>
    <div className={`max-w-[75%] rounded-2xl px-4 py-3 border ${m.role === "you" ? "bg-sky-500/15 border-sky-400/30" : "bg-white/6 border-white/10"}`}>
      <pre className="whitespace-pre-wrap text-sm break-words">{m.text}</pre>
        ...
        {/* === PROMPT BAR + MASCOT (card jadi relative) === */}
<div className="card relative overflow-visible space-y-3">
  ...
  <div className="flex items-end gap-2">
    ...
    {/* TEXTAREA auto-grow + no scrollbar */}
    <textarea
      ref={taRef}
      rows={1}
  ...
  {/* === MASCOT: kecil, pojok kanan atas panel prompt === */}
  <Image
  ...
  className="
    ...
    w-[clamp(40px,8vmin,96px)]  /* << lebih kecil & responsif */
