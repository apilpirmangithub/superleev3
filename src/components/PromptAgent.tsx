...
↗ View: ${explorerBase}/tx/${tx.hash}`);
      ...
      } catch (e: any) { push("agent", `Swap error: ${e?.message || String(e)}`); setToast("Swap error ❌"); }
      return;
    }
    ...
    if (intent.kind === "register") {
      try {
        ...
        const res = await client.ipAsset.mintAndRegisterIp({
          spgNftContract: (process.env.NEXT_PUBLIC_SPG_COLLECTION || "0xc32A8a0FF3beDDDa58393d022aF433e78739FAbc") as `0x${string}`,
...
↗ View: ${explorerBase}/tx/${res.txHash}`);
      ...
      } catch (e: any) { push("agent", `Register error: ${e?.message || String(e)}`); setToast("Register error ❌"); }
    }
  }
